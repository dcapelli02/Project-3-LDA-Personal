...
data plot_mianalyze;
    ...
    /* Variabili di riferimento (Baseline/Medie) */
    SEX_val = 0;   
    AGE_val = 0;   /* Standarizzato */
    BMI_val = 0;   /* Standarizzato */
    ADL_val = 10;  
    TAU_val = 0;   /* Standarizzato */
    ABP_val = 0;   /* Standarizzato */

    do TIME = 0 to 6 by 0.1;
        /* Ricostruzione del modello completo */
        /* NB: L'ordine deve rispecchiare quello di 'modeleffects' in PROC MIANALYZE */
        eta = Beta1                          /* Intercept */
            + (Beta2 * TIME)                 /* TIME */
            + (Beta3 * TIME**2)              /* TIME*TIME */
            + (Beta4 * SEX_val)              /* SEX */
            + (Beta5 * AGE_val)              /* AGE_STD */
            + (Beta6 * BMI_val)              /* BMI_STD */
            + (Beta7 * ADL_val)              /* ADL */
            + (Beta8 * TAU_val)              /* TAUPET_STD */
            + (Beta9 * ABP_val)              /* ABPET_STD */
            + (Beta10 * TIME * SEX_val)      /* TIME*SEX */
            + (Beta11 * TIME * AGE_val)      /* TIME*AGE_STD */
            + (Beta12 * TIME * BMI_val)      /* TIME*BMI_STD */
            + (Beta13 * TIME * ADL_val)      /* TIME*ADL */
            + (Beta14 * TIME * TAU_val)      /* TIME*TAU_STD */
            + (Beta15 * TIME * ABP_val)      /* TIME*ABP_STD */
            + (Beta16 * TIME**2 * SEX_val)   /* TIME2*SEX */
            + (Beta17 * TIME**2 * AGE_val)   /* TIME2*AGE_STD */
            + (Beta18 * TIME**2 * BMI_val)   /* TIME2*BMI_STD */
            + (Beta19 * TIME**2 * ADL_val)   /* TIME2*ADL */
            + (Beta20 * TIME**2 * TAU_val)   /* TIME2*TAU_STD */
            + (Beta21 * TIME**2 * ABP_val);  /* TIME2*ABP_STD */
